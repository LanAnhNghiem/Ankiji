package com.jishin.ankiji.utilities;

/**
 * Created by LanAnh on 14/01/2018.
 */

public class Constants {
    public static final String SET_BY_USER = "SetByUser";
    public static final String MOJI_NODE = "moji";
    public static final String KANJI_NODE = "data_kanji";
    public static final String MOJI_SET_NODE = "MojiSet";
    public static final String KANJI_SET_NODE = "KanjiSet";
    public static final String MOJI_SETS_NODE = "MojiSets";
    public static final String KANJI_SETS_NODE = "KanjiSets";
    public static final String USER_LIST_NODE = "UserList";
    public static final String CREATE_KANJI = "kanji";
    public static final String CREATE_MOJI = "moji";
    public static final String DATA_TYPE = "DATA_TYPE";
    public static final String USER_ID = "userID";
    public static final String SET_BY_USER_NODE = "SetByUser";
    public static final String CREATE = "create";
    public static final String NAME = "name";
    public static final String USER_NODE = "User";
    public static final String DATA_FILE = "data.json";
    public static final String CHART = "chart";
    public static final String CORRECT_ANSWER = "CorrectAnswser";
    public static final String TEST_TIMES = "TestTimes";
    public static final String DATE_SET_NODE = "DateSet";
    public static final String DATABASE_NAME = "ankiji-data";
    public static final String KANJI = "1";
    public static final String MOJI = "0";
}